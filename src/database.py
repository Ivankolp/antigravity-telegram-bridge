...
class Database:
    ...
    def get_chat_state(self, chat_id: int, chats_root: Path) -> ChatState:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT chat_dir, has_session, model, mode, effort, photo_enabled, turn_count, conversation_id
        ...
        with conn:
            conn.execute("""
                INSERT INTO chat_states (
                    ...
                    photo_enabled, turn_count, conversation_id, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def migrate_from_json_if_empty(self, json_path: Path, chats_root: Path) -> None:
        ...
        for k, v in chats_raw.items():
            try:
                cid = int(k)
                cs = ChatState(
                    chat_dir=v.get("chat_dir", str(chats_root / str(cid))),
                    has_session=bool(v.get("has_session", False)),
                    model=str(v.get("model", "")),
                    mode=str(v.get("mode", "code")),
                    effort=str(v.get("effort", "")),
                    photo_enabled=bool(v.get("photo_enabled", True)),
